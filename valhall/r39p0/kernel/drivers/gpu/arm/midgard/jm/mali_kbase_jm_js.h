 ...
 * need to do preemption on a slot, which includes stopping existing atoms
 * when a new atom is ready to run, and also which other atoms to remove from
 * the slot when the atom in JSn_HEAD is being preempted.
 *
 * This only handles @katom_a and @katom_b being for the same job slot, as
 * preemption only operates within a slot.
 *
