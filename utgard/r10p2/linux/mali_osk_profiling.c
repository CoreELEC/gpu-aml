...
const char *_mali_mem_counter_descriptions[] = _MALI_MEM_COUNTER_DESCRIPTIONS;
const char *_mali_special_counter_descriptions[] = _MALI_SPECIAL_COUNTER_DESCRIPTIONS;

static u32 current_profiling_pid = 0;

static void _mali_profiling_stream_list_destroy(mali_profiling_stream_list *profiling_stream_list)
{
...
_mali_osk_errcode_t _mali_osk_profiling_init(mali_bool auto_start)
{
	...
	for (i = 0; i < MALI_PROFILING_STREAM_BUFFER_NUM; i++) {
		...
		if (NULL == new_mali_profiling_stream) {
			_mali_profiling_stream_list_destroy(new_mali_profiling_stream_list);
			return _MALI_OSK_ERR_NOMEM;
		}
...
void _mali_osk_profiling_term(void)
{
	...
	if (NULL != global_mali_stream_list) {
		_mali_profiling_stream_list_destroy(global_mali_stream_list);
...
_mali_osk_errcode_t _mali_ukk_profiling_control_set(_mali_uk_profiling_control_set_s *args)
{
	...
	if (0 != control_packet_size) {
		...
		case PACKET_HEADER_START_CAPTURE_VALUE: {
			...
			/* Read stamping rate in nanoseconds and live rate, start capture.*/
