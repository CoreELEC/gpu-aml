	...
	if ((offset + mapping_page_num) == os_mem->count) {
		...
		list_for_each_entry_reverse(m_page, &os_mem->pages, list) {
			...
			if (mapping_page_num > 0) {
				...
				if (unlikely(ret & VM_FAULT_ERROR)) {
					MALI_DEBUG_PRINT(1, ("OS Mem: mali_mem_os_resize_cpu_map_locked failed, ret = 0x%x, offset is %d,page_count is %d\n",
		...
		list_for_each_entry(m_page, &os_mem->pages, list) {
			if (count >= offset) {
				...
				if (unlikely(ret & VM_FAULT_ERROR)) {
					MALI_DEBUG_PRINT(1, ("OS Mem: mali_mem_os_resize_cpu_map_locked failed, ret = 0x%x, count is %d, offset is %d,page_count is %d\n",
